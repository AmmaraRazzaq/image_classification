...
def validation_loop(val_loader, model, optimizer, rank, config, epoch, best_auc):
    ...
    with torch.no_grad():
        ...
        for i, val_batch in enumerate(val_loader):
            ...
            if i == 0:
                targetLabels = y.detach().cpu()
                predictedLabels = outputs_cls

            else:
                targetLabels = torch.cat((targetLabels, y.detach().cpu()), 0)
                predictedLabels = torch.cat((predictedLabels, outputs_cls), 0)
